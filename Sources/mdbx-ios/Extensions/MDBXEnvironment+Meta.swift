...
typealias MDBXEnvironmentReaderListHandler = (_ context: UnsafeMutableRawPointer?, _ num: Int32, _ slot: Int32, _ processId: pid_t, _ threadId: pthread_t?, _ txnId: UInt64, _ lag: UInt64, _ bytesUsed: Int, _ bytesRetained: Int) -> Int32
...
extension MDBXEnvironment {
    ...
    func getPath() throws -> String {
        var ut8Path = ("" as NSString).utf8String
        
        try withUnsafeMutablePointer(to: &ut8Path) { pointer in
        ...
        guard let ut8String = ut8Path, let path = NSString(utf8String: ut8String) else {
    ...
    func readerList(handler: @escaping MDBXEnvironmentReaderListHandler, context: inout Any) throws {
        try withUnsafeMutableBytes(of: &context) { pointer in
            let code = mdbx_reader_list(_env, self.cSetHandleReaderList({ (ctx, num, slot, processId, threadId, txnId, lag, bytesUsed, bytesRetained) -> Int32 in
                return handler(ctx, num, slot, processId, threadId, txnId, lag, bytesUsed, bytesRetained)
            }), pointer.baseAddress)
